# BMW CarData Streaming MQTT Bridge (v1.29.0)
...
* **Battery Health:** Derives a state-of-health figure from BMW's streamed usable energy (`batteryManagement.maxEnergy`) divided by the pack's nominal capacity. A daily value is kept **permanently** — outside the telemetry retention window — so degradation stays visible over years, with a trend line on the card and a chart behind it. Set `BATTERY_NOMINAL_KWH` for your model — there is no default, because the figure differs per model and a wrong one is worse than none. Computed as the 95th percentile over 30 days, since the raw value swings by several kWh during a day. Combustion cars are unaffected: BMW never streams the key there, so no row appears.
...
* **Batteriegesundheit:** Errechnet einen Gesundheitswert aus BMWs gestreamter nutzbarer Energie (`batteryManagement.maxEnergy`) geteilt durch die Nennkapazität. Ein Tageswert bleibt **dauerhaft** erhalten — außerhalb der Aufbewahrungsfrist der Telemetrie —, damit die Degradation über Jahre sichtbar bleibt: als Verlaufslinie in der Karte und als Diagramm dahinter. Diese wird über `BATTERY_NOMINAL_KWH` je Modell eingestellt — ohne Vorgabewert, denn sie ist bei jedem Fahrzeug anders, und eine falsche Zahl wäre schlechter als keine. Gerechnet wird über das 95. Perzentil von 30 Tagen, weil der Rohwert im Tagesverlauf um mehrere kWh schwankt. Reine Verbrenner sind nicht betroffen: Dort streamt BMW den Wert nie, es entsteht keine Zeile.
