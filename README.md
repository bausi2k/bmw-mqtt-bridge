# BMW CarData Streaming MQTT Bridge (v1.28.0)
...
* **Battery Health:** Derives a state-of-health figure from BMW's streamed usable energy (`batteryManagement.maxEnergy`) divided by the pack's nominal capacity. Set `BATTERY_NOMINAL_KWH` for your model — there is no default, because the figure differs per model and a wrong one is worse than none. Computed as the 95th percentile over 30 days, since the raw value swings by several kWh during a day. Combustion cars are unaffected: BMW never streams the key there, so no row appears.
