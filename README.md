# BMW CarData Streaming MQTT Bridge (v1.18.0)
...
* **Daily Budget Aware:** BMW allows 50 API calls per day, reset at 00:00 UTC. Responses are cached in SQLite and **survive a container restart** — before v1.18.0 every deploy re-bought the same data, measured at 13 calls within 16 seconds of startup. Concurrent requests for the same data share a single fetch instead of each paging through BMW's charging history; two overlapping requests once cost 28 calls for what a user experienced as one click. The stored age is the time of the fetch, not of the load, so a restart cannot silently extend the cache lifetime.
* **Charging History:** A dedicated tab lists your charging sessions — start, location, energy drawn, state of charge from/to, time on the cable and the power measured **while charging**. That last one matters: energy divided by total plug-in time collapses to a fraction once the car sits full on the cable, so the figure is taken from BMW's charging blocks instead. Filter by 7/14/30/45 days — the shorter ranges filter locally and cost no API call. **AC and DC are told apart** from the peak block power; BMW ships no field for it, so this is a derivation and labelled as one. Click a row for peak power, actual charging time, block count, odometer and preconditioning. Sessions where nothing reached the battery — BMW reports some as 0 kWh and some with no value at all — can be hidden with the **0 kWh** toggle; it filters locally, costs no API call, and remembers your choice. It is off by default, and when on, the number of hidden sessions is stated above the table. Note that the summary follows the filter: the total stays the same, but the average rises, becoming "per actual charge" rather than "per plug-in". Fetched once a day from BMW's `chargingHistory` endpoint, following BMW's pagination since v1.15.0.
