# BMW CarData Streaming MQTT Bridge (v1.14.1)
...
* **BMW CarData API Integration:** Fetches the official vehicle image and a full vehicle profile — model, series, body, colour, build date, drivetrain, head unit, SIM status and the complete option-code list. `basicData` is fetched daily anyway; until v1.13.0 only two of its 22 fields were shown.
...
* **Charging History:** A dedicated tab lists your charging sessions — start, location, energy drawn, state of charge from/to, time on the cable and the power measured **while charging**. That last one matters: energy divided by total plug-in time collapses to a fraction once the car sits full on the cable, so the figure is taken from BMW's charging blocks instead. Filter by 7/14/30/45 days — the shorter ranges filter locally and cost no API call. **AC and DC are told apart** from the peak block power; BMW ships no field for it, so this is a derivation and labelled as one. Click a row for peak power, actual charging time, block count, odometer and preconditioning. Fetched once a day from BMW's `chargingHistory` endpoint.
